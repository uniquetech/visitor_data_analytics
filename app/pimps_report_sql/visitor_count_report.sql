...
and DATE_TRUNC('day', event_datetime) >= TO_DATE('2014-09-06','YYYY-MM-DD') -1)) as inactive_user,
--reactivated users
(
...
from public.visitor_session_events vse
where domain_userid is not null
and DATE_TRUNC('day', event_datetime) = TO_DATE('2014-09-06','YYYY-MM-DD') -1)) as reactive_user,
...
where DATE_TRUNC('day', vs1.event_datetime) = TO_DATE('2014-09-06','YYYY-MM-DD')
and date_trunc('day', vs2.event_datetime) = TO_DATE('2014-09-06','YYYY-MM-DD') - 1) as active_users,
--churned users
(select count(distinct domain_userid) as churned_user
from public.visitor_session_events vs
where DATE_TRUNC('day',  event_datetime) = TO_DATE('2014-09-06','YYYY-MM-DD') -1
and domain_userid not in
(select distinct domain_userid
from public.visitor_session_events vs
where DATE_TRUNC('day', event_datetime) = TO_DATE('2014-09-06','YYYY-MM-DD'))) as churned_user